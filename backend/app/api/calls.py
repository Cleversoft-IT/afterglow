...
def derive_audio_diarization(
    simulation_config: Optional[dict[str, Any]],
    caller_mode: Optional[str],
) -> Optional[str]:
    """Look up the ASR diarization mode declared on the active template
    for the given simulator caller scenario.

    Returns one of `'speaker'` / `'channel'` if the scenario explicitly
    set it (TTS-stereo templates declare `'channel'`), otherwise `None`
    (the orchestrator then falls back to the default `'speaker'`).

    Kept as a small helper so the routing logic can be unit-tested
    without spinning up FastAPI + an HTTP client + a DB session.
    """
    if caller_mode is None or simulation_config is None:
        return None
    scenarios = simulation_config.get("scenarios") or {}
    scenario = scenarios.get(caller_mode) or {}
    value = scenario.get("audio_diarization")
    if value in ("speaker", "channel"):
        return value
    # Back-compat with the legacy flat shape: templates generated before
    # the scenarios refactor stored `audio_diarization` on the root.
    flat = simulation_config.get("audio_diarization")
    if flat in ("speaker", "channel"):
        return flat
    return None
...
async def submit_audio_call(
    ...
    phone_e164: str = Form(...),
    caller_mode: Optional[str] = Form(default=None),
    ctx: SessionContext = Depends(get_session_context),
    session: AsyncSession = Depends(get_session),
) -> CallSubmittedResponse:
    """`caller_mode` is the simulator scenario the operator pressed
    (`existing` or `new`). The backend uses it to look up the matching
    `simulation_config.scenarios.<mode>.audio_diarization` so the ASR
    call later routes to channel diarization for TTS-stereo audio."""
    if caller_mode is not None and caller_mode not in ("existing", "new"):
        raise HTTPException(
            status_code=400,
            detail="caller_mode must be 'existing' or 'new' when provided",
        )
    ...
    # Resolve which diarization mode the ASR should use for this audio.
    # TTS-rendered stereo (generated by `speechmatics_tts.render_script_to_mp3`)
    # carries `audio_diarization='channel'` on the matching simulation
    # scenario; everything else defaults to `'speaker'` downstream
    # (`orchestrator.run_pipeline` passes `call.audio_diarization or 'speaker'`).
    audio_diarization = derive_audio_diarization(
        template.simulation_config, caller_mode
    )
