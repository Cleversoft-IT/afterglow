...
async def submit_audio_call(
    ...
    # scenario; everything else defaults to `'speaker'`.
    audio_diarization: Optional[str] = None
    if caller_mode is not None:
        scenarios = (template.simulation_config or {}).get("scenarios") or {}
        scenario = scenarios.get(caller_mode) or {}
        scenario_diarization = scenario.get("audio_diarization")
        if scenario_diarization in ("speaker", "channel"):
            audio_diarization = scenario_diarization
