...
class Call(Base):
    ...
    # ASR diarization mode for this call's audio. `'speaker'` (default for
    # seed and user-uploaded audio) tells Speechmatics to use speaker
    # diarization on a mono track; `'channel'` is set for stereo audio
    # generated by the TTS pipeline (operator left, caller right) where
    # Speechmatics can pick the speakers apart by channel. The orchestrator
    # routes the ASR call based on this field — see
    # `app/agents/orchestrator.py` and `app/integrations/speechmatics.py`.
    audio_diarization: Mapped[Optional[str]] = mapped_column(
