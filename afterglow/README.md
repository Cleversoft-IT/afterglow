...
FastAPI background task ─► Speechmatics batch (diarization + lang detect + custom dict)
       ...
       │       Gemini-via-ADK picks which auto-mode tools to call.
       │       Falls back deterministically to analysis.planned_actions on failure.
       │       Mode (agentic | fallback) is recorded in audit_log.
       │
...
Missing Gemini key falls back to a deterministic stub of `CallAnalysis`.
Missing Vultr key skips RAG retrieval and write-back (the Postgres briefing
is still saved). The three demo MP3s under `app/assets/audio/` are pre-built
EN UK/US recordings generated by Speechmatics TTS — regenerate them with
