...
def _system_instruction(language: str, catalog_keys: list[str]) -> str:
    ...
    return (
        ...
        "{key, label, type, pii_class} for each field to extract; keys must "
        "be snake_case.\n"
        ...
        "    5. `dictionary_terms` — 5–15 domain-specific words the ASR "
        "should know.\n"
        "    6. `special_rules` — optional 1–3 `{when, then}` prompt hints.\n"
