...
**Eccezione nota: residui remoti nei servizi esterni.** Postgres è disposable, ma le collection Vultr Vector Store accumulate dai boot precedenti restano in vita: `vultr_inference` non espone un endpoint delete. Quando una migration cambia gli UUID5 delle seed Call (es. `0017_reshape_busy_week.py`), i chunk del round precedente restano nella collection Vultr remota con `phone` ancora valido, e `lookup_customer_memory` può pescarli mescolati ai nuovi. Trade-off accettato per la deadline hackathon: il rischio "stale RAG" è documentato qui invece di portare codice di cleanup remoto. La via pulita è creare una nuova collection in Vultr dashboard e aggiornare `VULTR_VECTOR_DEFAULT_COLLECTION` in Coolify env per `afterglow-backend`.
