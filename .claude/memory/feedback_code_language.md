...
Tutto Afterglow è in **inglese**: codice (commenti, identificatori, stringhe user-facing in UI, label dei menu, prompt agenti, log, messaggi d'errore, valori enum intent/sentiment) **e dati di seed/demo** (transcript di esempio, memory_summary dei clienti seed, prompt_hints, nomi dei business demo, customer phone numbers).
...
**How to apply:**
- Quando genero/modifico codice in `backend/**`, `app/**` (Expo) o `demo-site/**` (Vite): scrivo in inglese (label, commenti, stringhe).
