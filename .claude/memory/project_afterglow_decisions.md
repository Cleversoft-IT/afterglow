...
**N. Web first-paint sync + hydration guard (module-level + in-component in `_layout.tsx`).** Due meccanismi cooperanti per il cold load web:

1. **Module-level theme sync** (fuori dal componente): su `Platform.OS === 'web'` legge `readStoredThemePreference()` + `prefers-color-scheme` e setta `document.documentElement.style.colorScheme` + `document.body.style.backgroundColor` PRIMA del primo render React. Minimizza il flash di sfondo bianco al cold load (non lo elimina del tutto: serverebbe customizzare il template HTML di Expo per il vero pre-paint). Tocca solo nodi DOM che React non controlla via JSX → safe per l'hydration. Un `useEffect` ri-applica al cambio runtime. **Wording onesto**: "earliest possible from JS", non "pre-paint".

2. **Hydration guard in-component** (aggiunto 2026-05-17 sera, fix React error #418): `app.json` ha `web.output: "static"`, quindi Expo prerenderizza ogni route in HTML al build. Il build non ha `localStorage`/`matchMedia` → ramo "default" sempre. Se il primo render client diverge dal markup statico, React logga #418 (hydration mismatch). Pattern: `const [hydrated, setHydrated] = useState(false)` + `useEffect(() => setHydrated(true), [])`; ogni state derivato da `localStorage`/`window` (`isDark`, `gateChecked`/`isDemoMode()`) usa `hydrated ? realValue : buildTimeDefault`. Costa una transizione di re-render post-mount, elimina #418. **Quando aggiungi un nuovo state al root layout che dipende da browser APIs, deve passare per `hydrated`** — non scrivere mai `useState(readLocalStorage())` direttamente nel root layout.

Vedi [[feedback-web-first-paint]] per dettagli e regole.
