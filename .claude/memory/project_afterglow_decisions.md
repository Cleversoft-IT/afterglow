...
- HTTPS: Traefik + Let's Encrypt. DNS canonical = wildcard A `*.afterglow.cleversoft.it` → `95.179.245.107` (zona SiteGround). Le app espongono `api.afterglow.cleversoft.it`, `app.afterglow.cleversoft.it` e la demo pubblica canonica `afterglow.cleversoft.it`; `demo.afterglow.cleversoft.it` resta solo alias della demo. Gli hostname `<dashes>.sslip.io` originari restano come alias di fallback su ciascuna Application Coolify durante la transizione; possono essere rimossi quando la migrazione è validata in giudicare.
...
Secondo giro di test su `app.afterglow.cleversoft.it` post-rewrite ha esposto bug bloccanti di navigazione + gap di forma rispetto al pitch "sostituto del dialer Pixel". Cluster di 8 stage chiusi in due commit (`d155c52` + `6823f1d`). Le decisioni qui sono permanenti.
...
- **`CORS_ORIGINS`** (CSV) sul backend: `https://app.95-179-245-107.sslip.io,https://demo.95-179-245-107.sslip.io,https://95-179-245-107.sslip.io,https://app.afterglow.cleversoft.it,https://demo.afterglow.cleversoft.it,https://afterglow.cleversoft.it` (aggiornato 2026-05-19; `afterglow.cleversoft.it` è la demo pubblica canonica, `demo.afterglow.cleversoft.it` resta alias, sslip resta fallback). Sostituisce `AFTERGLOW_CORS_EXTRA_ORIGINS` (eliminata).
