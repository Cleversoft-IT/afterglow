...
**N. Web first-paint sync (module-level in `_layout.tsx`).** Codice module-level (fuori dal componente) su `Platform.OS === 'web'` legge `readStoredThemePreference()` + `prefers-color-scheme` e setta `document.documentElement.style.colorScheme` + `document.body.style.backgroundColor` PRIMA del primo render React. Minimizza il flash di sfondo bianco al cold load (non lo elimina del tutto: serverebbe customizzare il template HTML di Expo per il vero pre-paint). Un `useEffect` ri-applica al cambio runtime. **Wording onesto**: questo è "earliest possible from JS", non "pre-paint" — il vero pre-paint richiede `app.json web.template` custom, fuori scope.
