...
- HTTPS: Traefik + Let's Encrypt. DNS canonical = wildcard A `*.afterglow.cleversoft.it` → `95.179.245.107` (zona SiteGround). Le 3 app espongono `api.afterglow.cleversoft.it`, `app.afterglow.cleversoft.it`, `demo.afterglow.cleversoft.it` (cert LE HTTP-01 per ciascuna). Gli hostname `<dashes>.sslip.io` originari restano come alias di fallback su ciascuna Application Coolify durante la transizione; possono essere rimossi quando la migrazione è validata in giudicare.
