...
3. **English code, English seed/demo data, Italian conversation.** Codice, UI strings, comments, log messages, **seed data** (`backend/app/db/seed.py`), demo customer profiles, demo transcripts — all in English. The bundled demo MP3s under `app/assets/audio/` are EN UK/US (Speechmatics TTS preview voices). The Italian-only surface is the user-Claude conversation. See [`feedback_code_language.md`](.claude/memory/feedback_code_language.md).
