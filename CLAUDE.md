...
2. **AI runs post-call, not during the call.** Operator UI reads `customer.memory_summary` from Postgres with zero latency. The post-call pipeline is: (a) a single Gemini structured-output call in `backend/app/agents/call_analyzer.py` does extraction, classification, planned actions and the next-call briefing in one shot; (b) `backend/app/agents/pii_sanitizer.py` applies per-`pii_class` redaction to the briefing + planned-action evidence **before** anything is persisted (audit step `pii_policy_applied`); (c) `backend/app/agents/action_planner.py` re-reads the SANITIZED analysis through Google ADK and emits typed tool calls (Pydantic models built dynamically from each action's `payload_schema`); (d) `backend/app/executors/action_executor.py` revalidates each payload with `jsonschema.validate`, refuses empty-evidence actions when `evidence_required=True`, flags `mutates=True` actions in the audit, and stamps `mock: True` on the result of every action whose target lives in `MOCK_REGISTRY` so the UI renders a "Simulated" badge; (e) `_persist_memory` writes a bilingual chunk (native + EN summary) to the Vultr Vector Store in prod (skipped in demo). **Fail-fast everywhere**: missing key, Gemini error, ADK error, schema mismatch → `Call.status="failed"` + audit row with `status="error"`. No deterministic stub / fallback. Every step is recorded in `audit_log`, with `input_tokens` / `output_tokens` populated from `usage_metadata` for Gemini calls and from the `usage` block for Vultr RAG.
3. **English code, English seed/demo data, Italian conversation.** Codice, UI strings, comments, log messages, **seed data** (`backend/app/db/seed.py`), demo customer profiles, demo transcripts — all in English. The bundled demo MP3s under `app/assets/audio/` are EN UK/US (Speechmatics TTS preview voices). Each of the three domain presets ships TWO MP3s — `<domain>_existing.mp3` (returning caller, references shared history) and `<domain>_new.mp3` (first-time caller, full self-introduction) — wired to the Simulator's "existing" / "new" buttons. The Italian-only surface is the user-Claude conversation. See [`feedback_code_language.md`](.claude/memory/feedback_code_language.md).
