...
- **`.claude/memory/`** — project memory, versioned & team-shared.
  ...
  - [`feedback_web_first_paint.md`](.claude/memory/feedback_web_first_paint.md) — the module-level theme sync in `_layout.tsx` minimizes the cold-load flash from JS; pair it with the in-component hydration guard (`hydrated` flag + `useEffect`), because `web.output: "static"` prerenders without `localStorage`/`matchMedia` and raw first-render reads cause React error #418. Do not claim "pre-paint" without a custom Expo HTML template.
